(function () {
  ...
  function init() {
      ...
      <section id="fontPreviewApp" class="previewApp">
        ...
        <section class="controlCard">
          ...
          <div class="adjustGrid">
            <label class="rangeField"><span>Cỡ chữ</span><div><button data-size-step="-10" type="button">−</button><input id="sizeRange" type="range" min="24" max="300" value="120"><input id="sizeNumber" type="number" min="24" max="300" value="120"><button data-size-step="10" type="button">+</button></div></label>
        ...
        <section id="viewer" class="viewer">
          <div class="viewerHead">
            <div><small>LIVE PREVIEW</small><strong id="viewerMode">ENGLISH</strong><span id="viewerInfo">120px • giãn chữ 0px</span></div>
            <div class="viewerActions"><button id="zoomOut" type="button" title="Thu nhỏ">−</button><button id="zoomReset" type="button">100%</button><button id="zoomIn" type="button" title="Phóng to">+</button><button id="fullScreen" class="fullScreen" type="button">⛶ TOÀN MÀN HÌNH</button></div>
          </div>
    ...
    document.getElementById('zoomOut').addEventListener('click',()=>setSize(Number(ui.size.value)-10));document.getElementById('zoomIn').addEventListener('click',()=>setSize(Number(ui.size.value)+10));document.getElementById('zoomReset').addEventListener('click',()=>setSize(120));document.getElementById('fitWidth').addEventListener('click',fitWidth);document.getElementById('resetView').addEventListener('click',()=>{ui.color.value='#f4cf64';ui.bg.value='#000000';setSpace(0);setSize(120)});document.getElementById('clearText').addEventListener('click',()=>{setText('');ui.text.focus()});
  ...
  const style=document.createElement('style');style.textContent=`
    *{box-sizing:border-box}.previewOnlyBody{margin:0!important;background:#eaf0f7!important;color:#0f172a!important;font-family:Segoe UI,Arial,sans-serif!important}.previewMain{max-width:1480px;margin:auto;padding:20px}.previewApp{width:100%}
